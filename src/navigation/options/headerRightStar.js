...
const headerRightStar = (route, navigation) => {
    ...
    async function checkMouviForFavorites() {
      try {
        ...
        const isAdded = parsedFavoriteList.some(item => item.id === params.id);
        setIsFav(isAdded);
      } catch (e) {
        console.log(e);
      }
    ...
    async function deleteMouviFromStorage() {
      try {
        const favoriteList = await AsyncStorage.getItem('favoriteList');
        const parsedFavoriteList = favoriteList ? JSON.parse(favoriteList) : [];
        const newList = parsedFavoriteList.filter(item => item.id !== params.id);
    ...
    return (
      <TouchableOpacity
        onPress={() => (isFav ? deleteMouviFromStorage() : setMouviToStorage())}>
